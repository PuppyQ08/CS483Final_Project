...
#define MNIST_DATA_LOCATION "/ece408/mnist/"
...
{
    if(if_train)
        mnist_load(MNIST_DATA_LOCATION "train-images-idx3-ubyte", MNIST_DATA_LOCATION "train-labels-idx1-ubyte",
            &train_set, &train_cnt);
    else 
        mnist_load(MNIST_DATA_LOCATION "t10k-images-idx3-ubyte", MNIST_DATA_LOCATION "t10k-labels-idx1-ubyte",
            &test_set, &test_cnt);
}
