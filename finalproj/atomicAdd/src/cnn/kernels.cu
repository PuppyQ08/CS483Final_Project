...
#include <cuda.h>
#include <cublas_v2.h>
//__constant__ float image[28][28];
...
__global__ void fp_preact_c1( float preact[6][24][24], float weight[6][5][5]){
    ...
    if (tx < N) { 
            atomicAdd(&preact[tx/5/5%6][tx/5/5/6%24][tx/5/5/6/24%24],
                weight_s[tx/5/5%6][tx%5][tx/5%5] * image[tx/5/5/6%24 + tx%5][tx/5/5/6/24%24 + tx/5%5]);
    }
}
