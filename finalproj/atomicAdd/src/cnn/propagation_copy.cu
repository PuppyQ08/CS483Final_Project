...
// __device__ __constant__ static float fp_preact_c1_input_const[28][28];
...
static double forward_propagation(double data[28][28])
{
    ...
    // float **fp_c1_input = l_input.output;
    // cudaMemcpyToSymbol(fp_preact_c1_input_const, input, 28*28*sizeof(float));
    fp_preact_c1<<<338, 256>>>((float (*)[28])l_input.output, (float (*)[24][24])l_c1.preact, (float (*)[5][5])l_c1.weight);

    fp_bias_c1<<<27, 128>>>((float (*)[24][24])l_c1.preact, l_c1.bias);
    apply_activation_function<<<64, 64>>>(l_c1.preact, l_c1.output, l_c1.O);
    
    fp_preact_s1<<<14, 16>>>((float (*)[24][24])l_c1.output, (float (*)[6][6])l_s1.preact, (float (*)[4][4])l_s1.weight);
...
static double back_propagation()
{
    ...
    // # threads must be >= max(l_f.M * l_f.N, l_f.O, l_s1.O)
    //n_threads = max(l_f.M * l_f.N, l_f.O, l_s1.O);
    //n_blocks = ceil((float)n_threads/(float)BLOCK_SIZE);
    bp_weight_f<<<64, 64>>> ((float (*)[6][6][6])l_f.d_weight, l_f.d_preact, (float (*)[6][6])l_s1.output);
    bp_bias_f<<<64, 64>>>(l_f.bias, l_f.d_preact);
    //change from <<14,16>> to <<64,64>> to fix timing and accuracy frops issue
    bp_output_s1<<<64, 64>>>((float (*)[6][6])l_s1.d_output, (float (*)[6][6][6])l_f.weight, l_f.d_preact);
    bp_preact_s1<<<64, 256>>>((float (*)[6][6])l_s1.d_preact, (float (*)[6][6])l_s1.d_output, (float (*)[6][6])l_s1.preact);
    ...
    bp_weight_c1<<<338, 256>>>((float (*)[5][5])l_c1.d_weight, (float (*)[24][24])l_c1.d_preact, (float (*)[28])l_input.output);
