...
__global__ void fp_preact_c1(float input[28][28], float preact[6][24][24], float weight[6][5][5]){
    ...
    if (tx < N) {
        for (int i = 0; i < 5*5; ++i) {
            preact[tx/24/24][tx/24%24][tx%24] +=
                weight_s[tx/24/24][i/5][i%5] * input[tx/24%24 + i/5][tx%24 + i%5];
        }
    }
}
...
__global__ void fp_preact_s1(float input[6][24][24], float preact[6][6][6], float weight[1][4][4]){
    ...
    const int tix = threadIdx.x;
    const int N = 6*6*6;
    //__shared__ float weight_s[1][4][4];
    //if (tix < 4*4){
    //    weight_s[tix/4/4][tix%16/4][tix%4] = weight[tix/4/4][tix%16/4][tix%4];
    //}
    //__syncthreads();
...
__global__ void fp_preact_f(float input[6][6][6], float preact[10], float weight[10][6][6][6]){
    const int tx = blockIdx.x * blockDim.x + threadIdx.x;
    //const int tix = threadIdx.x;
    //__shared__ float weight_s[10][6][6][6];
    //if (tix < 10*6*6*6){
    //    //weight_s[tix/6/6/6][tix%216/36][tix%36/6][tix%36%6] = weight[tix/6/6/6][tix%216/36][tix%36/6][tix%36%6];
    //    weight_s[tix/6/6/6][tix%216/36][tix%36%6][tix%36/6] = weight[tix/6/6/6][tix%216/36][tix%36%6][tix%36/6];
    //}
    //__syncthreads();
    ...
    if (tx < 10) {
        for (int i = 0; i < 6*6*6; ++i) {
            preact[tx] += weight[tx][i/6/6][i/6%6][i%6] * input[i/6/6][i/6%6][i%6];
        }
    }
}
...
__global__ void bp_output_s1(float d_output[6][6][6], float n_weight[10][6][6][6], float nd_preact[10]){
    ...
    if(tx < N){
        ...
        for(int i4 = 0; i4 < 10; i4++){
            d_output[i1][i2][i3] += n_weight[i4][i1][i2][i3] * nd_preact[i4];
        }
...
__global__ void bp_preact_s1(float d_preact[6][6][6], float d_output[6][6][6], float preact[6][6][6]){
    ...
    const int N = 6*6*6;
    //const int tix = threadIdx.x;
    //__shared__ float d_output_s[6][6][6];
    //if (tix < 6*6*6){
    //    d_output_s[tix/6/6][tix%36/6][tix%36%6] = d_output[tix/6/6][tix%36/6][tix%36%6];
    ////    weight_s[tix/6/6/6][tix%216/36][tix%36%6][tix%36/6] = weight[tix/6/6/6][tix%216/36][tix%36%6][tix%36/6];
    //}
    //__syncthreads();
    if (tx < N) {
...
__global__ void bp_weight_s1(float d_weight[1][4][4], float d_preact[6][6][6], float p_output[6][24][24]){
    ...
    if (tx < N) {
        for (int i = 0; i < 6*6*6; ++i) {
            d_weight[0][tx/4][tx%4] += 
                d_preact[i%6][i/6%6][i/6/6%6] * p_output[i%6][i/6%6 * 4 + tx/4][i/6/6%6 * 4 + tx%4];
        }
    }
...
__global__ void bp_bias_s1(float bias[1], float d_preact[6][6][6]){
    ...
    //const int tix = threadIdx.x;
    //__shared__ float d_preact_s[6][6][6];
    //if (tix < 6*6*6){
    //    d_preact_s[tix/6/6][tix%36/6][tix%36%6] = d_preact[tix/6/6][tix%36/6][tix%36%6];
    ////    weight_s[tix/6/6/6][tix%216/36][tix%36%6][tix%36/6] = weight[tix/6/6/6][tix%216/36][tix%36%6][tix%36/6];
    //}
    //__syncthreads();
...
__global__ void bp_weight_c1(float d_weight[6][5][5], float d_preact[6][24][24], float p_output[28][28]){
    ...
    if (tx < N) {
        for (int i = 0; i < 24*24; ++i) {          
            d_weight[tx/5/5][tx/5%5][tx%5] +=
                d_preact[tx/5/5][i/24][i%24] * p_output[i/24 + tx/5%5][i%24 + tx%5] / d;
        }
    }
...
__global__ void bp_bias_c1(float bias[6], float d_preact[6][24][24]){
    ...
    if (tx < N) {
        for(int i = 0; i < 24*24; i++){
            bias[tx] += dt * d_preact[tx][i/24][i%24] / d;
        }
    }
